...
BYTE* shiftRows(BYTE *block, int mode){ 
    ...
    switch(mode){
        ...
        case ENC:
            /*********************************************** { 구현 4 시작 } ********************************************/
			for(i = (KEY_SIZE/4); i < 4 + (KEY_SIZE/4) - 1; i++)// row 2
			{
				temp = block[i];
				block[i] = block[i+1];
				block[i+1] = temp;
			}
			for(i = (KEY_SIZE/4) * 2; i < 8 + (KEY_SIZE/4) - 2; i++) // row 3
			{
				temp = block[i];
				block[i] = block[i+2];
				block[i+2] = temp;
			}
			for(i = 12 + (KEY_SIZE/4) - 1; i > (KEY_SIZE/4) * 3; i--) //row4
			{
				temp = block[i];
				block[i] = block[i-1];
				block[i-1] = temp;
